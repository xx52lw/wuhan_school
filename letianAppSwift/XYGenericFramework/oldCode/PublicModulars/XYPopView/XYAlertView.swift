...
typealias XYpopViewActType =  () -> Void
class XYAlertView: UIView {
    
    
    enum actBtnStatusEnum {
        case highlight
        case normal
    }
   ...
   private var control:UIControl!
    ...
    let btnToDetailHeight = cmSizeFloat(12)
    let btnHeight = cmSizeFloat(35)
    
    //点击背景是否收回View
    var clickControlHideView:Bool = true
    
    init(frame: CGRect,detailTipsStr:String,titleStr:String,cancelStr:String?,certainStr:String,cancelBtnClosure:XYpopViewActType?, certainClosure:@escaping XYpopViewActType) {
        ...
        let detailStrHeight = detailTipsStr.stringHeight(sheetViewWidth - textToSheetViewSide*2, font: cmSystemFontWithSize(13))
        let sheetViewHeight = titleToTop*2 + cmSingleLineHeight(fontSize: cmBoldSystemFontWithSize(17)) + detailStrHeight + btnHeight + btnToDetailHeight*2
        ...
        detailTipsLabel  = UILabel(frame: CGRect(x: textToSheetViewSide, y: titleLabel.bottom + titleToTop, width: sheetViewWidth - textToSheetViewSide*2, height: detailStrHeight))
        detailTipsLabel.textColor = MAIN_BLACK2
        detailTipsLabel.font = cmSystemFontWithSize(13)
        detailTipsLabel.textAlignment = .center
        detailTipsLabel.numberOfLines = 0
        detailTipsLabel.text = detailTipsStr
        sheetView.addSubview(detailTipsLabel)
        
        if cancelclosure == nil {
            certainBtn = UIButton(frame: CGRect(x: btnToSheetSide, y: detailTipsLabel.bottom + btnToDetailHeight, width: sheetViewWidth*3/5, height: btnHeight))
            ...
            cancelBtn = UIButton(frame: CGRect(x: 0, y: detailTipsLabel.bottom + btnToDetailHeight, width: sheetViewWidth/2, height: btnHeight))
            cancelBtn.setTitle(cancelStr, for: .normal)
            cancelBtn.setTitleColor(MAIN_BLACK, for: .normal)
            ...
            certainBtn = UIButton(frame: CGRect(x: sheetViewWidth/2, y: detailTipsLabel.bottom + btnToDetailHeight, width: sheetViewWidth/2, height: btnHeight))
            certainBtn.setTitle(certainStr, for: .normal)
            certainBtn.setTitleColor(MAIN_BLACK, for: .normal)
            certainBtn.titleLabel?.font = cmSystemFontWithSize(15)
            certainBtn.addTarget(self, action: #selector(certainBtnAct), for: .touchUpInside)
            sheetView.addSubview(certainBtn)
        }
