...
- (UIImage *)getImageWithName:(NSString *)name {
    ...
    NSString *path = [[[NSBundle mainBundle] pathForResource:@"SVProgressHUD.bundle" ofType:nil] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.png",name]];
...
- (NSMutableArray *)animationArray {
    if (_animationArray == nil) {
        ...
        for (NSInteger i = 1; i <= 8; ++i) {
            UIImage *image = [self getImageWithName:[NSString stringWithFormat:@"%ld",i]];
            if (image != nil) {
                [_animationArray addObject:image];
            }
        }
    }
...
//        _indefiniteAnimatedLayer = nil;
        if (self.imageView.isAnimating == YES) {
...
//    self.imageView.image= [UIImage imageWithGIFNamed:@"SVProgressHUD.bundle/loading"];
    self.imageView.animationImages = self.animationArray;
    self.imageView.animationRepeatCount = 1000000;
    self.imageView.animationDuration = 1.0f;
    self.imageView.frame = CGRectMake(0.0f, 0.0f, 80.0f, 80.0f);
    self.imageView.center = arcCenter;
