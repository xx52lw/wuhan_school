...
class StaffOrderDetailVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate {
    ...
    var chooseStatusTextField:UITextField!
    //确认/更换按钮
    var  btnView:UIView!
    var setDeliveryStatusBtn:UIButton!
    
    
    //网络异常空白页
    public var staffOrderAbnormalView:XYAbnormalViewManager!
    
    
    var orderId:String!
    var oderDetailModel:StaffOrderDetailModel!
    var service:StaffOrderDetailService = StaffOrderDetailService()
    ...
    override func viewDidLoad() {
        ...
        creatNavTopView()
        if orderId != nil {
            self.service.staffOrderDetailDataRequest(target: self, orderID: orderId)
        }
    ...
    public func creatAbnormalView(isNetError:Bool){
        ...
        if isNetError == true{
            staffOrderAbnormalView = abnormalView
            staffOrderAbnormalView.abnormalType = .networkError
            //数据错误刷新Act
            staffOrderAbnormalView.refreshActionBlock = {[weak self] in
                self?.service.staffOrderDetailDataRequest(target: self!, orderID: self!.orderId)
            }
        }
    }
    ...
    func createStaffSubviewsLabel(){
        ...
        var leftActTimeStr = showTimeType +   String(oderDetailModel.leftLimitTime/60) + "分钟"
        var attributeStrRange:NSRange = NSMakeRange(6, leftActTimeStr.count - 6)

        
        if oderDetailModel.HasArrive == true {
            leftActTimeStr = "配送用时: " +   String(oderDetailModel.DeliveryUseTime) + "分钟"
            attributeStrRange = NSMakeRange(5, leftActTimeStr.count - 5)
        }
        ...
        staffSetTitleLabel.textColor = MAIN_BLUE
        staffSetTitleLabel.textAlignment = .left
        staffSetTitleLabel.text = "送达状态"
        tableHeaderView.addSubview(staffSetTitleLabel)
        
        let staffNameTitleStr = "送达状态:"
        ...
        chooseStatusTextField = UITextField(frame: CGRect(x: staffNameLabel.right + toside, y: staffNameLabel.frame.origin.y, width: SCREEN_WIDTH   - staffNameTitleStrWidth - toside*2 , height: textLineHeight))
        chooseStatusTextField.font = cmSystemFontWithSize(14)
        chooseStatusTextField.textColor = MAIN_BLACK
        chooseStatusTextField.textAlignment = .right
        chooseStatusTextField.backgroundColor = .white
        chooseStatusTextField.placeholder = "请选择"
        
        let showMoreImageView = UIImageView(frame: CGRect(x: chooseStatusTextField.frame.size.width - toside*2 - showMoreImage.size.width, y: textLineHeight/2 - showMoreImage.size.height/2, width: showMoreImage.size.width + toside*2, height: showMoreImage.size.height))
        showMoreImageView.image = showMoreImage
        showMoreImageView.contentMode = .scaleAspectFit
        chooseStatusTextField.rightView = showMoreImageView
        chooseStatusTextField.rightViewMode = .always
        chooseStatusTextField.delegate = self
        tableHeaderView.addSubview(chooseStatusTextField)
        ...
        btnView = UIView(frame: CGRect(x: 0, y: chooseStatusTextField.bottom, width: SCREEN_WIDTH, height: btnViewHeight))
        btnView.backgroundColor = seperateLineColor
        tableHeaderView.addSubview(btnView)
        
        setDeliveryStatusBtn = UIButton(frame: CGRect(x: toside, y: cmSizeFloat(10), width: SCREEN_WIDTH - toside*2, height: cmSizeFloat(40)))
        setDeliveryStatusBtn.setTitle("确  定", for: .normal)
        setDeliveryStatusBtn.setTitleColor(MAIN_WHITE, for: .normal)
        setDeliveryStatusBtn.titleLabel?.font = cmSystemFontWithSize(15)
        setDeliveryStatusBtn.layer.cornerRadius = cmSizeFloat(4)
        setDeliveryStatusBtn.clipsToBounds = true
        setDeliveryStatusBtn.backgroundColor = MAIN_GREEN
        setDeliveryStatusBtn.addTarget(self, action: #selector(setDeliveryStatusBtnAct), for: .touchUpInside)
        btnView.addSubview(setDeliveryStatusBtn)
        
        
        //如果订单已经送达
        if oderDetailModel.HasArrive == true {
            chooseStatusTextField.text = oderDetailModel.ArriveStatus
//            chooseStatusTextField.isEnabled = false
//            showMoreImageView.isHidden = true
//            setDeliveryStatusBtn.isEnabled = false
//            setDeliveryStatusBtn.backgroundColor = .gray
        }
    ...
    //MARK: - 设置送达状态按钮响应
    @objc func setDeliveryStatusBtnAct(){
        
        if chooseStatusTextField.text!.trimmingCharacters(in: .whitespaces).isEmpty {
            cmShowHUDToWindow(message: "请先选择配送状态")
            return
        }
        
        service.staffSetDeliveryStatusRequest(userOrderID: self.orderId, ArriveStatus: chooseStatusTextField.text!, successAct: { [weak self] in
            cmShowHUDToWindow(message: "设置成功")
            let navVCCount = self!.navigationController!.viewControllers.count
            if  let lastVC = self?.navigationController?.viewControllers[navVCCount - 2] as? StaffOrderListVC {
    ...
    func creatMainTabView(){
        ...
        mainTabView.register(StaffOrderDetailTabcell.self, forCellReuseIdentifier: "StaffOrderDetailTabcell")
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return StaffOrderDetailTabcell.cellHeight
    }
    
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        
        var cell = tableView.dequeueReusableCell(withIdentifier: "StaffOrderDetailTabcell")
        if cell == nil {
            cell = StaffOrderDetailTabcell(style: UITableViewCellStyle.default, reuseIdentifier: "StaffOrderDetailTabcell")
        }
        if let targetCell = cell as? StaffOrderDetailTabcell{
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        if textField ==  chooseStatusTextField{
            let chooseStatusVC = ChooseDeliveryStatusVC()
            self.navigationController?.pushViewController(chooseStatusVC, animated: true)
            return false
        }
