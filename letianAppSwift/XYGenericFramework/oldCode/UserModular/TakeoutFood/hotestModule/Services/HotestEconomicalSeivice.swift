...
import UIKit
import SwiftyJSON
class HotestEconomicalSeivice: NSObject {
    ...
    //MARK: - 请求经济实惠数据
    func hotestEconomicalDataRequest(target: UIViewController){
        if let economicalMealVC  = target as? HotestSetMenuVC {
            self.currentPage = 1
            
            let urlParameters = hotestEconomicalUrl + "/\(self.currentPage)"
    ...
    //MARK: - 经济实惠列表页上拉加载
    func getHotestEconomicalPullListData(target: UIViewController){
        if let economicalMealVC  = target as? HotestSetMenuVC {
            ...
            //如果不是最后一页则进行数据请求
            let urlParameters = hotestEconomicalUrl + "/\(requestPage)"
