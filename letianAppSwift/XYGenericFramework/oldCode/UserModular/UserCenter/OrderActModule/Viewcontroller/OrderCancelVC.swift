...
class OrderCancelVC: UIViewController {
    ...
    let subviewTitleArr:[String] = ["联系商家，快速取消订单","在线取消订单"]
    let subviewTipsArr:[String] = ["订单在处理中，建议您先与商家电话沟通。","如因自身原因取消订单，商家有权拒绝"]
    ...
    @objc func showMoreBtnAct(sender:UIButton) {
        ...
        case 200:
            ...
            break
        case 201:
            let cancelOnlineVC = OrderCancelOnlineVC()
            cancelOnlineVC.orderID = self.orderID
            self.navigationController?.pushViewController(cancelOnlineVC, animated: true)
            break
        default:
            break
        }
