//
//  MerchantReturnOrderDetailVC.swift
//  XYGenericFramework
//
...
class MerchantReturnOrderDetailVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate {
    ...
    //同意及拒绝按钮按钮
    var  btnView:UIView!
    var agreeReturnBtn:UIButton!
    var refusedReturnBtn:UIButton!
    ...
    func creatNavTopView(){
        ...
        if oderDetailModel.TDType == .refund{
            topView.titleLabel.text = "申请退款"
        }else{
          topView.titleLabel.text = "取消订单"
        }
    ...
    func refreshSubviewUI(){
        if oderDetailModel.chooseStaffModel != nil {
            orderStatusTextField.text = oderDetailModel.chooseStaffModel.StaffName
            agreeReturnBtn.setTitleColor(MAIN_WHITE, for: .normal)
            agreeReturnBtn.backgroundColor = MAIN_GREEN
        }else{
            orderStatusTextField.text = ""
            agreeReturnBtn.setTitleColor(MAIN_WHITE, for: .normal)
            agreeReturnBtn.backgroundColor = .gray
        }
    }
    ...
    func createHeaderInfoSubviews(){
        ...
        var  leftActTimeStr = "距超时还剩: " +   String(oderDetailModel.leftLimitTime/60) + "分钟"
        var attributeStrRange:NSRange = NSMakeRange(6, leftActTimeStr.count - 6)
        if oderDetailModel.TDType == .refund{
            leftActTimeStr = "回复剩余时间: " +   String(oderDetailModel.leftLimitTime/60) + "分钟"
            attributeStrRange = NSMakeRange(7, leftActTimeStr.count - 7)
        }
        ...
        //订单状态View
        var orderStatusTitleStr = "订单状态:"
        
        if oderDetailModel.TDType == .refund {
            orderStatusTitleStr  = "送达状态:"
        }
        ...
        if oderDetailModel.TDType == .refund {
            ...
            orderStatusTextField.text = oderDetailModel.OrderStatus
        }
        ...
        if oderDetailModel.TDType == .refund {
            returnReasonTitleLabel.text = "申请退款原因"
        }else{
            returnReasonTitleLabel.text = "取消订单原因"
        }
        ...
        let btnSpaceWidht = cmSizeFloat(25)
        let btnHeight = cmSizeFloat(36)
        let btnWidth = (SCREEN_WIDTH - btnSpaceWidht - toside*2)/2
        ...
        agreeReturnBtn = UIButton(frame: CGRect(x: toside, y: (btnViewHeight - btnHeight)/2, width: btnWidth, height: btnHeight))
        if oderDetailModel.TDType == .refund {
            agreeReturnBtn.setTitle("同意退款", for: .normal)
        }else{
            agreeReturnBtn.setTitle("同意退单", for: .normal)
        }
        agreeReturnBtn.setTitleColor(MAIN_WHITE, for: .normal)
        agreeReturnBtn.titleLabel?.font = cmSystemFontWithSize(15)
        agreeReturnBtn.layer.cornerRadius = cmSizeFloat(4)
        agreeReturnBtn.clipsToBounds = true
        agreeReturnBtn.backgroundColor = MAIN_GREEN
        agreeReturnBtn.addTarget(self, action: #selector(agreeReturnBtnAct), for: .touchUpInside)
        btnView.addSubview(agreeReturnBtn)
        
        refusedReturnBtn  = UIButton(frame: CGRect(x: agreeReturnBtn.right + btnSpaceWidht, y: (btnViewHeight - btnHeight)/2, width: btnWidth, height: btnHeight))
        if oderDetailModel.TDType == .refund {
            refusedReturnBtn.setTitle("拒绝退款", for: .normal)
        }else{
            refusedReturnBtn.setTitle("拒绝退单", for: .normal)
        }
        refusedReturnBtn.setTitleColor(MAIN_WHITE, for: .normal)
        refusedReturnBtn.titleLabel?.font = cmSystemFontWithSize(15)
        refusedReturnBtn.layer.cornerRadius = cmSizeFloat(4)
        refusedReturnBtn.clipsToBounds = true
        refusedReturnBtn.backgroundColor = cmColorWithString(colorName: "F4A460")
        refusedReturnBtn.addTarget(self, action: #selector(refusedReturnBtnAct), for: .touchUpInside)
        btnView.addSubview(refusedReturnBtn)
        
    }
    
    
    //MARK: - 同意退单按钮响应
    @objc func agreeReturnBtnAct(){
        
        returnOrderPopView = XYNoDetailTipsAlertView(frame: .zero, titleStr: "你确定要同意退单吗？", cancelStr: "取消", certainStr: "确定", cancelBtnClosure: {
            
        }, certainClosure: { [weak self] in
            self?.agreeReturnBtn.isEnabled = false
            
            self?.service.returnOrderMerchantActRequest(userOrderID: self!.oderDetailModel.orderNumber, isAgree: true, reason: "", successAct: {
                cmShowHUDToWindow(message: "同意退单成功")
                self?.navigationController?.popViewController(animated: true)
            }, failureAct: {
                self?.agreeReturnBtn.isEnabled = true
            })
    ...
    //MARK: - 拒绝退单按钮响应
    @objc func refusedReturnBtnAct(){
        
        let refuseReturnOrderVC = MerchantRefuseReturnOrderVC()
        refuseReturnOrderVC.orderID = self.oderDetailModel.orderNumber
        self.navigationController?.pushViewController(refuseReturnOrderVC, animated: true)
