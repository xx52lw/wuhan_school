...
class OrderCancelOnlineVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextViewDelegate {

    private  var topView:XYTopView!
    private let toside = cmSizeFloat(20)
    private let subviewHeight = cmSizeFloat(50)
    private let reasonStrArr = ["配送时间过长","其他原因"]
    ...
    func createTableviewFooter(){
        ...
        let placeholderText = "请输入取消原因，2-100字以内"
    ...
    @objc func submitBtnAct(){
        ...
        service.applicationForReturnRequest(userOrderID: self.orderID, content: cancelReasonText, successAct: {
            cmShowHUDToWindow(message: "提交成功")
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5, execute: { [weak self] in
                //self?.navigationController?.popViewController(animated: true)
                
                let navVCCount = self!.navigationController!.viewControllers.count
                if  let targetVC = self?.navigationController?.viewControllers[navVCCount - 4] as? OrderManageVC {
