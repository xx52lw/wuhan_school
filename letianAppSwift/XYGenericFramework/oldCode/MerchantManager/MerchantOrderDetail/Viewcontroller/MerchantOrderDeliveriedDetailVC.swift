//
//  MerchantOrderDeliveriedDetailVC.swift
//  XYGenericFramework
//
...
class MerchantOrderDeliveriedDetailVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
    
    let useTimeLabelHeight = cmSizeFloat(50)
    ...
    override func viewDidLoad() {
        ...
        creatNavTopView()
        if orderId != nil {
            self.service.merchantDeliveriedOrderDetailRequest(target: self, orderID: orderId)
        }
    ...
    public func creatAbnormalView(isNetError:Bool){
        ...
        if isNetError == true{
            ...
            merchantOrderAbnormalView.refreshActionBlock = {[weak self] in
                self?.service.merchantDeliveriedOrderDetailRequest(target: self!, orderID: self!.orderId)
            }
        }
    }
    ...
    func createStaffSubviewsLabel(){
        ...
        useTimeLabel = UILabel(frame: CGRect(x: 0, y: 0, width: SCREEN_WIDTH, height: textLineHeight))
        useTimeLabel.textAlignment = .center
        useTimeLabel.font = cmSystemFontWithSize(14)
        useTimeLabel.textColor = MAIN_BLACK
        
        
        let leftActTimeStr = "配送用时: " +   String(oderDetailModel.DeliveryUseTime) + "分钟"
        ...
        attributeStr.addAttribute(NSAttributedStringKey.foregroundColor, value: MAIN_BLACK, range: attributeStrRange)
        useTimeLabel.attributedText = attributeStr
        tableHeaderView.addSubview(useTimeLabel)
        
        
        let staffSeperateView = XYCommonViews.creatCustomSeperateLine(pointY: useTimeLabel.bottom, lineWidth: SCREEN_WIDTH, lineHeight: seperateLineHeight)
    ...
    func creatTableHeaderView() {
        ...
        let tableHeaderViewHeight = textLineHeight*6 + receiverInfoHeight + orderGenerationInfoHeight + textToTopBottomHeight*4 + seperateLineHeight*5 + useTimeLabelHeight
