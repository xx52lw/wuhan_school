...
@interface WMPageController () <horizontalScrollViewDelegate> {
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if (![scrollView isKindOfClass:WMScrollView.class]) return;
    //当开始横向滑动时，禁止竖直方向滑动
    if (self.horizontalScrollDelegate != nil) {
    [self.horizontalScrollDelegate unableContainerScroll];
    }
    _startDragging = YES;
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    //当横向滑动拖拽结束时，打开竖直方向滑动
    if (self.horizontalScrollDelegate != nil) {
        [self.horizontalScrollDelegate enableContainerScroll];
    }
