...
class PasswordSettingVC: UIViewController,UITextFieldDelegate {

    let strWidth = "用户账户".stringWidth(cmSingleLineHeight(fontSize: cmSystemFontWithSize(15)), font: cmSystemFontWithSize(15))
    let toside = cmSizeFloat(20)
    let subViewHeight = cmSizeFloat(50)
    let textSpace = cmSizeFloat(10)
    
    var topView:XYTopView!
    var userAccountTextfield:UITextField!
    ...
    func createSubviews() {
        ...
        titleLabel.text = "用户账号"
        self.view.addSubview(titleLabel)
        
        
        userAccountTextfield = UITextField(frame: CGRect(x: strWidth + toside + textSpace, y: titleLabel.top, width: SCREEN_WIDTH - strWidth - toside*2 - textSpace, height: subViewHeight))
        userAccountTextfield.font = cmSystemFontWithSize(15)
        userAccountTextfield.textColor = MAIN_BLACK
        userAccountTextfield.textAlignment = .left
        userAccountTextfield.returnKeyType = .done
        userAccountTextfield.isEnabled = false
        if userSettingInfoModel.account != nil {
            userAccountTextfield.text = userSettingInfoModel.account
        }
        self.view.addSubview(userAccountTextfield)
        
        self.view.addSubview(XYCommonViews.creatCommonSeperateLine(pointY: userAccountTextfield.bottom - cmSizeFloat(1)))
        
        //初始密码
        let passwordTitleLabel = UILabel(frame: CGRect(x: toside, y: userAccountTextfield.bottom, width: strWidth, height: subViewHeight))
        ...
        passwordModifyTitleLabel.text = "更改密码"
    ...
    @objc func submitModifyBtnAct(sender:UIButton) {
        ...
        paramsDict["ConfirmPassword"] = passWordModifyTextfield.text!
        
        service.updatePassWordRequest(paramsDict: paramsDict, successAct: { [weak self] in
