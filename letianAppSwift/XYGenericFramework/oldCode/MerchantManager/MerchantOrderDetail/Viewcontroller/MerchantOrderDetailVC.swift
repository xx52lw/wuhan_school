//
//  MerchantCurrentOrderDetailVC.swift
//  XYGenericFramework
//
...
class MerchantCurrentOrderDetailVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        createBottomView()
        if orderId != nil {
            self.service.merchantCurrentOrderDetailDataRequest(target: self, orderID: orderId)
        }
    ...
    public func creatAbnormalView(isNetError:Bool){
        ...
        if isNetError == true{
            ...
            merchantOrderAbnormalView.refreshActionBlock = {[weak self] in
                self?.service.merchantCurrentOrderDetailDataRequest(target: self!, orderID: self!.orderId)
            }
        }
    }
    ...
    func createStaffSubviewsLabel(){
        ...
        changeOrOkBtn.setTitle("确  定", for: .normal)
        changeOrOkBtn.setTitleColor(MAIN_WHITE, for: .normal)
    ...
    @objc func changeOrOkBtnAct(){
        ...
        if oderDetailModel.chooseStaffModel == nil {
            cmShowHUDToWindow(message: "请先选择配送员")
            return
        }
        
        self.service.setStaffRequest(userOrderID: oderDetailModel.orderNumber, dStaffID: oderDetailModel.chooseStaffModel.DStaffID, successAct: { [weak self] in
            cmShowHUDToWindow(message: "已发送给该配送员")
            DispatchQueue.main.async {
                self?.changeOrOkBtn.setTitle("更换", for: .normal)
                self?.changeOrOkBtn.isEnabled = false
                self?.chooseStaffTextField.isEnabled = false
                self?.changeOrOkBtn.backgroundColor = .gray
