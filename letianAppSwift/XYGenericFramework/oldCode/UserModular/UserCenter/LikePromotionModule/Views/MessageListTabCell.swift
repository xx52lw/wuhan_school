...
class MessageListTabCell: UITableViewCell {
    ...
    var cellModel:MessageListCellModel!
    ...
    @objc func  deleteBtnAct() {
        if let currentVC = GetCurrentViewController() as? MessagesListVC {
            
            currentVC.service.deleteMessageRequest(messageID: cellModel.BroadcastReceiveID, successAct: {
                //cmShowMbProgressHUD(message: "删除成功")
                currentVC.service.getMessageListRequest(target: currentVC)
            }, failureAct: {
    ...
    //MARK: - 设置Model
    func setModel(model:MessageListCellModel){
