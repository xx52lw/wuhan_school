...
class EvaluateAnswerTabCell: UITableViewCell {
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        if !self.isEqual(nil) {
            ...
            answerView  = UIView(frame: .zero)
            answerView.backgroundColor = cmColorWithString(colorName: "cccccc")
            self.addSubview(answerView)
            
            answerContentLabel = UILabel(frame: .zero)
            answerContentLabel.font = UIFont.systemFont(ofSize: 13)
            answerContentLabel.textColor =  MAIN_BLACK2
            answerContentLabel.numberOfLines = 0
            answerContentLabel.backgroundColor = .clear
    ...
    func setModel(model:EvaluateCellModel){
        ...
        let answerContentViewWidth = SCREEN_WIDTH - avatarBtn.right - userNameToAvatarWidth - toLeftSide
        let answerContentViewHeiht =  model.answerContent.stringHeight(commentContentWidth, font: cmSystemFontWithSize(13)) + cmSizeFloat(6)
        
        answerView.frame = CGRect(x: avatarBtn.frame.maxX + userNameToAvatarWidth, y: commentContentLab.bottom + gradeToCommentHeight, width: answerContentViewWidth, height: answerContentViewHeiht)
        
        answerContentLabel.frame = CGRect(x: cmSizeFloat(4), y: cmSizeFloat(3), width: answerContentViewWidth - cmSizeFloat(4), height: answerContentViewHeiht - cmSizeFloat(6))
        answerContentLabel.text = model.answerContent
        
        
        sperateLine.frame.origin.y = answerView.bottom + gradeToCommentHeight
